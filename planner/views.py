...
from .functions import srecords_to_df, get_preplot_shots, get_preplot_coordinates
import pandas as pd
...
class HomeView(View):
    ...
    def get(self, request):
        ...
        for sequence in sequences:
            first_point = AcquisitionShotPoint.objects.filter(sequence=sequence).order_by('datetime').first()
            last_point = AcquisitionShotPoint.objects.filter(sequence=sequence).order_by('datetime').last()
...
class LoadPreplotView(View):
    ...
    def get(self, request):
        ...
        if preplot is None:
            messages.error(request, 'No preplot data available. Please upload a preplot file first.')
            return render(request, 'planner/load_preplot.html')
        elif preplot.loaded:
            messages.success(request, 'Preplot file already loaded.')
            return redirect('home')

        return render(request, 'planner/load_preplot.html')  # Added this to display the form if no errors
    ...
    def post(self, request):
        ...
        messages.success(request, f'Successfully loaded {preplot_file.name}. Processed {len(preplot_lines)} new lines.')
        ...
        preplot_line_map = {pl.preplot: pl for pl in PreplotLine.objects.filter(preplot__in=preplot_lines_dict.keys())}
        ...
        messages.success(request, f'Successfully processed {len(preplot_lines)} lines and {len(preplot_shotpoints)} shotpoints.')
    ...
    def post(self, request):
        try:
            ...
            for sequence_file in sequence_files:
                ...
                if Sequence.objects.filter(sequence_number=sequence_file.name).exists():
